///|/ Copyright (c) Prusa Research 2019 - 2023 Enrico Turri @enricoturri1966, Oleksandra Iushchenko
///@YuSanka, Lukáš Matěna @lukasmatena, Tomáš Mészáros @tamasmeszaros, Filip Sykala @Jony01, Lukáš
/// Hejl @hejllukas, Vojtěch Bubník @bubnikv, Vojtěch Král @vojtechkral
...
// TODO: 1.出去之后再进来，原先的point的cache没删干净
//       2.point_id选取的时候有问题
...
IconManager::Icons init_icons(IconManager &mng, ImVec2 size = ImVec2{50, 50}) {
    ...
    IconManager::InitTypes init_types{
        {"support_structure_invisible.svg", size, IconManager::RasterType::color
        }, // show_support_points_selected
        {"support_structure_invisible.svg", size, IconManager::RasterType::gray_only_data
        }, // show_support_points_unselected
        {"support_structure_invisible.svg", size, IconManager::RasterType::color
        }, // show_support_points_hovered

        {"support_structure.svg", size, IconManager::RasterType::color
        }, // show_support_structure_selected
        {"support_structure.svg", size, IconManager::RasterType::gray_only_data
        }, // show_support_structure_unselected
        {"support_structure.svg", size, IconManager::RasterType::color
        }, // show_support_structure_hovered
    };
...
bool draw_view_mode(bool &show_support_structure, const IconManager::Icons &icons) {
    ...
    if (show_support_structure) {
        ...
        ImGui::SameLine();
        if (clickable(
                get_icon(icons, IconType::show_support_points_unselected),
            ...
            ImGui::SetTooltip(
                "%s", _u8L("Click to show support points without support structure").c_str()
            );
...
GLGizmoSlaSupports::GLGizmoSlaSupports(
    GLCanvas3D &parent, const std::string &icon_filename, unsigned int sprite_id
)
...
void GLGizmoSlaSupports::on_render() {
    ...
    // m_parent.toggle_sla_auxiliaries_visibility(m_show_support_structure);
    ...
    //    // TODO:m_c->selection_info()->get_active_instance() !=
    //    selection.get_instance_idx()原本做了这个判断 if (m_state == On &&
    ...
    if (m_editing_mode && m_hover_id == -1 && m_hover_position.has_value()) {
        render_hover_sphere(*m_hover_position, *m_hover_normal);
    }  
...
void GLGizmoSlaSupports::render_points(const Selection &selection) {
    ...
    size_t now_point_id = 0;
    // 当前遍历的point_ID（在所有点的数组的id而不是每个object里点的id） 用于高亮选中点的
    // 注意添加点的投射器时的顺序要和这个一致
    for (auto const obj_idx : mo_idxs) {
        ModelObject *mo = selection.get_model()->objects[obj_idx];

        std::vector<CacheEntry> &n_editing_cache = m_editing_cache[obj_idx];
        std::vector<sla::SupportPoint> &n_normal_cache = m_normal_cache[obj_idx];
        const size_t cache_size = m_editing_mode ? n_editing_cache.size() : n_normal_cache.size();

        ModelInstance *inst = mo->instances[0];
        if (!inst)
            return;

        // double shift_z = m_c->selection_info()->print_object()->get_current_elevation();
        double shift_z =
            m_parent.sla_print()->get_print_object_by_model_object_id(mo->id())->get_current_elevation(
            );
        ...
        const Transform3d instance_scaling_matrix_inverse =
            transformation.get_scaling_factor_matrix().inverse();
        ...
        for (size_t i = 0; i < cache_size; ++i) {
            const sla::SupportPoint &support_point = m_editing_mode ?
                n_editing_cache[i].support_point :
                n_normal_cache[i];
            ...
            render_color = support_point.type == sla::SupportPointType::manual_add ?
                manual_color :
                support_point.type == sla::SupportPointType::island ?
                island_color :
                inactive_color;
            // First decide about the color of the point.
            // TODO:按map存，m_hover_id不会等于i
            if (m_hover_id != -1) {
                m_cone.model.set_color(render_color);
            }

            if (m_editing_mode) {
                if (size_t(m_hover_id) ==
                    now_point_id) // ignore hover state unless editing mode is active
                    render_color = hovered_color;
                else if (n_editing_cache[i].selected)
            ...
            // Inverse matrix of the instance scaling is applied so that the mark does not
            // scale with the object.
            const Transform3d support_matrix =
                Geometry::translation_transform(support_point.pos.cast<double>()) *
            ...
            if (m_editing_mode) {
                ...
                // if (n_editing_cache[i].normal == Vec3f::Zero())
                //    m_c->raycaster()->raycaster()->get_closest_point(
                //        n_editing_cache[i].support_point.pos, &n_editing_cache[i].normal
                //    );

                if (n_editing_cache[i].normal == Vec3f::Zero())
                    m_volume_raycasters[obj_idx]->get_raycaster()->get_closest_point(
                        n_editing_cache[i].support_point.pos, &n_editing_cache[i].normal
                    );
                ...
                q.setFromTwoVectors(
                    Vec3d::UnitZ(),
                    instance_scaling_matrix_inverse * n_editing_cache[i].normal.cast<double>()
                );
                const Eigen::AngleAxisd aa(q);
                const Transform3d model_matrix =
                    transformation.get_matrix() * support_matrix *
                    Transform3d(aa.toRotationMatrix()) *
                    Geometry::translation_transform(
                        (CONE_HEIGHT + support_point.head_front_radius * RenderPointScale) *
                        Vec3d::UnitZ()
                    ) *
                    Geometry::rotation_transform({double(PI), 0.0, 0.0}) *
                    Geometry::scale_transform({CONE_RADIUS, CONE_RADIUS, CONE_HEIGHT});
            ...
            const Transform3d model_matrix = transformation.get_matrix() * support_matrix *
                Geometry::scale_transform(radius);
            shader->set_uniform("view_model_matrix", view_matrix * model_matrix);
            const Matrix3d view_normal_matrix = view_matrix.matrix().block(0, 0, 3, 3) *
                model_matrix.matrix().block(0, 0, 3, 3).inverse().transpose();
            shader->set_uniform("view_normal_matrix", view_normal_matrix);
...
//新增球体渲染方式
void GLGizmoSlaSupports::render_hover_sphere(const Vec3f &position, const Vec3f &normal) {
    GLShaderProgram *shader = wxGetApp().get_shader("gouraud_light");
    if (!shader)
        return;
    ...
    ModelObject *mo = selection.get_model()->objects[m_hover_mo_id];
    ...
    double shift_z = po ? po->get_current_elevation() : 0.0;
    ...
    // 半透明黄色表示预览
    const ColorRGBA hover_color = ColorRGBA(1.0f, 1.0f, 0.0f, 0.5f);
    m_sphere.model.set_color(hover_color);
    shader->set_uniform("emission_factor", 0.5f);

    // 与 render_points() 相同的变换逻辑
    const Transform3d instance_scaling_matrix_inverse =
        transformation.get_scaling_factor_matrix().inverse();
    const Transform3d support_matrix = Geometry::translation_transform(position.cast<double>()) *
        instance_scaling_matrix_inverse;
    const double radius = (double) m_new_point_head_diameter / 2.0 * RenderPointScale;
    const Transform3d model_matrix = transformation.get_matrix() * support_matrix *
        Geometry::scale_transform(radius);
...
bool GLGizmoSlaSupports::is_mesh_point_clipped(const Vec3d &point) const {
    ...
    const Transform3d &trafo = mi->get_transformation().get_matrix() *
        sel_info->model_object()->volumes.front()->get_matrix();
...
bool GLGizmoSlaSupports::gizmo_event(
    ...
    // int active_inst = m_c->selection_info()->get_active_instance();
     ...
     // ===== 新增:在函数开头持续更新鼠标位置和悬停状态 =====  
     //m_mouse_pos = mouse_position;  

     //if (m_editing_mode && m_hover_id == -1) {
     //    std::pair<Vec3f, Vec3f> pos_and_normal;
     //    if (unproject_on_mesh(mouse_position, pos_and_normal)) {
     //        m_hover_position = pos_and_normal.first;
     //        m_hover_normal = pos_and_normal.second;
     //        m_parent.request_extra_frame(); // 请求重绘以显示悬停球体
     //    } else {
     //        m_hover_position.reset();
     //        m_hover_normal.reset();
     //    }
     //} else {
     //    // 如果悬停在已有点上或不在编辑模式,清除悬停预览
     //    m_hover_position.reset();
     //    m_hover_normal.reset();
     //}  
    ...
    if (m_editing_mode) {
        ...
        if (action == SLAGizmoEventType::LeftDown && (shift_down || alt_down || control_down)) {
            ...
            // auto hovered_idx_ = static_cast<size_t>(m_hover_volumn_id);
            // const GLVolume *gl_volume = gl_volumes[hovered_idx_];
            // m_hover_mo_id = gl_volume->object_idx();
            if (m_hover_id == -1) {
                if (shift_down || alt_down) {
                    m_selection_rectangle.start_dragging(
                        mouse_position,
                        shift_down ? GLSelectionRectangle::EState::Select :
                                     GLSelectionRectangle::EState::Deselect
                    );
                }
            } else {
                // temp_hover_id 当前悬停处的object的m_editing_cache里，点的下标
                std::set<unsigned int> mo_idxs = m_parent.get_selection().get_object_idxs();
                size_t temp_hover_id = m_hover_id;
                auto temp_cache_id = mo_idxs.begin();
                while (temp_hover_id > m_editing_cache[m_hover_mo_id].size()) {
                    // temp_hover_id -= m_editing_cache[mo_idxs[temp_cache_id++]].size();
                    temp_hover_id -= m_editing_cache[*temp_cache_id++].size();
                }

                if (m_editing_cache[m_hover_mo_id][temp_hover_id].selected)
        ...
        if (action == SLAGizmoEventType::LeftDown && !m_selection_rectangle.is_dragging() &&
            !shift_down) {
            // If any point is in hover state, this should initiate its move - return control back
            // to GLCanvas:
            if (m_hover_id != -1)
                return false;
            if (m_hover_volumn_id == -1)
                return false;
            ...
            if (m_selection_empty) {
                ...
                std::set<unsigned int> mo_idxs = m_parent.get_selection().get_object_idxs();
                if (mo_idxs.find(m_hover_mo_id) == mo_idxs.end()) {
                    m_hover_mo_id = -1;
                    return false;
                }
                ...
                if (unproject_on_mesh(mouse_position, pos_and_normal)) { // we got an intersection
                    ...
                    m_editing_cache[m_hover_mo_id].emplace_back(
                        sla::SupportPoint{pos_and_normal.first, m_new_point_head_diameter / 2.f},
                        false, pos_and_normal.second
                    );
        ...
        // action == SLAGizmoEventType::AltUp) && m_selection_rectangle.is_dragging()) {
        //    // Is this a selection or deselection rectangle?
        //    GLSelectionRectangle::EState rectangle_status = m_selection_rectangle.get_state();

        //    // First collect positions of all the points in world coordinates.
        //    Geometry::Transformation trafo = mo->instances[active_inst]->get_transformation();
        //    trafo.set_offset(trafo.get_offset() + Vec3d(0., 0.,
        //    m_c->selection_info()->get_sla_shift())); std::vector<Vec3d> points; for (unsigned int
        //    i=0; i<m_editing_cache.size(); ++i)
        //        points.push_back(
        //            trafo.get_matrix() *
        //            m_editing_cache[m_old_mo_id][i].support_point.pos.cast<double>()
        //        );
        ...
        //            (trafo.get_matrix().cast<float>() *
        //             (m_editing_cache[m_old_mo_id][idx].support_point.pos +
        //              m_editing_cache[m_old_mo_id][idx].normal))
        //                .cast<float>()
        //        );
        ...
        if (action == SLAGizmoEventType::DiscardChanges) {
            ask_about_changes(
                [this]() { editing_mode_apply_changes(); },
...
void GLGizmoSlaSupports::delete_selected_points(bool force) {
    ...
    /*for (unsigned int idx = 0; idx < m_editing_cache.size(); ++idx) {
        if (m_editing_cache[m_hover_mo_id][idx].selected &&
            (!m_editing_cache[m_hover_mo_id][idx].support_point.is_island() ||
             !m_lock_unique_islands || force)) {
            m_editing_cache[m_hover_mo_id].erase(m_editing_cache[m_hover_mo_id].begin() + (idx--));
        }
    }*/
    const Selection &selection = m_parent.get_selection();
    std::set<unsigned int> mo_idxs = selection.get_object_idxs();
    for (const auto obj_id : mo_idxs) {
        for (unsigned int idx = 0; idx < m_editing_cache[obj_id].size(); ++idx) {
            if (m_editing_cache[obj_id][idx].selected &&
                (!m_editing_cache[obj_id][idx].support_point.is_island() ||
                 !m_lock_unique_islands || force)) {
                m_editing_cache[obj_id].erase(m_editing_cache[obj_id].begin() + (idx--));
            }
        }
    }
...
std::vector<const ConfigOption *> GLGizmoSlaSupports::get_config_options(
    const std::vector<std::string> &keys, ModelObject *mo
) const {
...
void GLGizmoSlaSupports::on_render_input_window(float x, float y, float bottom_limit) {
    ...
    if (float line_height = ImGui::GetTextLineHeightWithSpacing();
        m_icons.empty() || rendered_line_height != line_height) { // change of view resolution
...
RENDER_AGAIN:
    ...
    // First calculate width of all the texts that are could possibly be shown. We will decide set
    // the dialog width based on that:

    const float settings_sliders_left = ImGuiPureWrap::calc_text_size(m_desc.at("points_density")).x +
        m_imgui->scaled(1.f);
    const float clipping_slider_left =
        std::max(
            ImGuiPureWrap::calc_text_size(m_desc.at("clipping_of_view")).x,
            ImGuiPureWrap::calc_text_size(m_desc.at("reset_direction")).x
        ) +
        m_imgui->scaled(1.5f);
    const float diameter_slider_left = ImGuiPureWrap::calc_text_size(m_desc.at("head_diameter")).x +
        m_imgui->scaled(1.f);
    const float minimal_slider_width = m_imgui->scaled(4.f);
    const float buttons_width_approx = ImGuiPureWrap::calc_text_size(m_desc.at("apply_changes")).x +
        ImGuiPureWrap::calc_text_size(m_desc.at("discard_changes")).x + m_imgui->scaled(1.5f);
    const float lock_supports_width_approx =
        ImGuiPureWrap::calc_text_size(m_desc.at("lock_supports")).x + m_imgui->scaled(2.f);

    float window_width = minimal_slider_width +
        std::max(std::max(settings_sliders_left, clipping_slider_left), diameter_slider_left);
    ...
    if (m_editing_mode) {
        // if (m_old_mo_id != -1) {
        //     ModelObject *mo = selection.get_model()->objects[m_old_mo_id];
        // }

        float diameter_upper_cap =
            static_cast<ConfigOptionFloat *>(
                wxGetApp().preset_bundle->sla_prints.get_edited_preset().config.option(
        ...
        // m_imgui->slider_float("##head_diameter", &m_new_point_head_diameter, 0.1f, diameter_upper_cap,
        // "%.1f"); if (m_imgui->get_last_slider_status().clicked) {
        ...
        if (draw_view_mode(m_show_support_structure, m_icons)) {
            ...
            m_parent.toggle_sla_auxiliaries_visibility(are_sla_supports_shown());
        ...
        for (const auto &id : mo_idxs) {
            ...
            ObjectID oid = mo->id();
            std::vector<CacheEntry> &n_editing_cache = m_editing_cache[id];
            std::vector<sla::SupportPoint> &n_normal_cache = m_normal_cache[id];
            const size_t cache_size = m_editing_mode ?
                n_editing_cache.size() :
                n_normal_cache.size();

            const char *support_points_density = "support_points_density_relative";
            float density = static_cast<const ConfigOptionInt *>(
                                get_config_options({support_points_density}, mo)[0]
            )
                                ->value;
            float old_density = density;
            wxString tooltip = _L("Change amount of generated support points.");
            if (m_imgui->slider_float(
                    "##density", &density, 50.f, 200.f, "%.f %%", 1.f, false, tooltip
                )) {
                if (density < 10.f) // not neccessary, but lower value seems pointless. Zero cause
                                    // issues inside algorithms.
                    density = 10.f;
            ...
            static std::optional<int>
                density_stash; // Value for undo/redo stack is written on stop dragging
            if (!density_stash.has_value() &&
                !is_approx(density, old_density)) // stash the values of the settings so we know
                                                  // what to revert to after undo
                density_stash = (int) old_density;
            if (density_status.deactivated_after_edit &&
                density_stash.has_value()) { //  slider released
            ...
            const sla::SupportPoints &supports = n_normal_cache;
            ...
            if (supports.empty()) {
                ...
                stats = GUI::format(
                    "%d support points generated (%d on islands)", (int) supports.size(),
                    count_island
                );
            } else {
                stats = GUI::format(
                    "%d(%d manual) support points (%d on islands)", (int) supports.size(),
                    count_user_edited, count_island
                );
            }
        ...
        if (float overhang_sample_distance = sample_config.prepare_config.discretize_overhang_step;
            m_imgui->slider_float(
                "overhang discretization", &overhang_sample_distance, 2e-5f, 10.f, "%.2f mm"
            )) {
            sample_config.prepare_config.discretize_overhang_step = overhang_sample_distance;
        } else if (ImGui::IsItemHovered())
            ImGui::SetTooltip("Smaller will slow down. Step for discretization overhang outline "
                              "for test of support need");
        ...
        // ImGuiPureWrap::text("");
        // ImGuiPureWrap::text(m_c->m_model_object->sla_points_status == sla::PointsStatus::NoPoints
        // ? _(L("No points  (will be autogenerated)")) :
        //              (m_c->m_model_object->sla_points_status == sla::PointsStatus::AutoGenerated
        //              ? _(L("Autogenerated points (no modifications)")) :
        ...
        //              sla::PointsStatus::Generating ? _(L("Generation in progress...")) : "UNKNOWN
        //              STATUS"))));
    }
...
void GLGizmoSlaSupports::draw_island_config() {
    ...
    if (float max_for_one = unscale<float>(sample_config.max_length_for_one_support_point
        ); // [in mm]
    ...
    if (float max_for_two = unscale<float>(sample_config.max_length_for_two_support_points
        ); // [in mm]
    ...
    if (float minimal_distance_from_outline = unscale<float>(
            sample_config.minimal_distance_from_outline
        ); // [in mm]
    ...
    ImGui::SameLine();
    if (float maximal_distance_from_outline = unscale<float>(
            sample_config.maximal_distance_from_outline
        ); // [in mm]
    ...
    if (float simplification_tolerance = unscale<float>(sample_config.simplification_tolerance
        ); // [in mm]
    ...
    if (float minimal_move = unscale<float>(sample_config.minimal_move); // [in mm]
        ...
        ImGui::SetTooltip(
            "Align termination condition, when support points after align did not change their "
            "position more,\n"
...
bool GLGizmoSlaSupports::on_is_activable() const {
    ...
    //// Check that none of the selected volumes is outside. Only SLA auxiliaries (supports) are
    /// allowed outside.
    // const Selection::IndicesList& list = selection.get_volume_idxs();
    // for (const auto& idx : list)
    //     if (selection.get_volume(idx)->is_outside &&
    //     selection.get_volume(idx)->composite_id.volume_id >= 0)
    //         return false;
...
void GLGizmoSlaSupports::on_set_state() {
    ...
    if (m_state == On) { // the gizmo was just turned on
        ...
        /*if (! selected_print_object_exists(m_parent, _L("Selected object has to be on the active
        bed."))) {
           m_state = Off;
           return;
       }*/
        ...
        m_new_point_head_diameter =
            static_cast<const ConfigOptionFloat *>(cfg.option("support_head_front_diameter"))->value;
        m_parent.toggle_sla_auxiliaries_visibility(m_show_support_structure);
        // disable_editing_mode();
        reload_cache();
        ...
        const Selection &selection = m_parent.get_selection();
        std::set<unsigned int> mo_idxs = selection.get_object_idxs();
        bool exist_unsaved_changes = false;
        for (auto const id : mo_idxs) {
            ModelObject *mo = selection.get_model()->objects[id];
            if (unsaved_changes(id)) {
                exist_unsaved_changes = true;
                break;
            }
        }

        if (m_editing_mode && exist_unsaved_changes && on_is_activable()) {
        ...
        m_parent.toggle_sla_auxiliaries_visibility(true);
...
void GLGizmoSlaSupports::on_start_dragging() {
    ...
    if (m_hover_id != -1 && m_hover_mo_id != -1) {
        select_point(NoPoints);
        select_point(m_hover_id);
        // size_t temp_cache_id = 0, temp_hover_id = m_hover_id;
        // while (temp_hover_id > m_editing_cache[m_hover_mo_id].size()) {
        //     temp_hover_id -= m_editing_cache[temp_cache_id++].size();
        // }
        ...
        m_point_before_drag = m_editing_cache[m_hover_mo_id][temp_hover_id];
    } else
...
void GLGizmoSlaSupports::on_stop_dragging() {
    if (m_hover_id != -1 && m_hover_mo_id != -1) {
        ...
        CacheEntry backup = m_editing_cache[m_hover_mo_id][temp_hover_id];

        if (m_point_before_drag.support_point.pos != Vec3f::Zero() // some point was touched
            && backup.support_point.pos !=
                m_point_before_drag.support_point.pos) // and it was moved, not just selected
        {
            m_editing_cache[m_hover_mo_id][temp_hover_id] = m_point_before_drag;
            Plater::TakeSnapshot snapshot(wxGetApp().plater(), _L("Move support point"));
            m_editing_cache[m_hover_mo_id][temp_hover_id] = backup;
        }
    }
    m_point_before_drag = CacheEntry();
}
...
void GLGizmoSlaSupports::on_dragging(const UpdateData &data) {
    assert(m_hover_id != -1 && m_hover_mo_id != -1);
    if (!m_editing_mode)
        return;
    ...
    if (m_editing_cache[m_hover_mo_id][temp_hover_id].support_point.is_island() &&
        m_lock_unique_islands)
        return;
    ...
    m_editing_cache[m_hover_mo_id][temp_hover_id].support_point.pos = pos_and_normal.first;
    m_editing_cache[m_hover_mo_id][temp_hover_id].support_point.type =
        sla::SupportPointType::manual_add;
    m_editing_cache[m_hover_mo_id][temp_hover_id].normal = pos_and_normal.second;
}
...
void GLGizmoSlaSupports::on_load(cereal::BinaryInputArchive &ar) {
    ...
    ar(m_new_point_head_diameter, m_selection_empty);
}
...
// TODO
bool GLGizmoSlaSupports::unproject_on_mesh(
    ...
    auto hovered_idx_ = static_cast<size_t>(m_hover_volumn_id);
    const GLVolume *gl_volume = gl_volumes[hovered_idx_];
    /*m_old_mo_id = gl_volume->object_idx();*/
    ...
    if (po) {
        double shift_z = po->get_current_elevation();
        trafo = inst->get_transformation().get_matrix();
        trafo.translation()(2) += shift_z;
    }
    ...
    if (m_volume_raycasters[m_hover_mo_id]->get_raycaster()->unproject_on_mesh(
            mouse_pos, trafo /*m_volumes.volumes.front()->world_matrix()*/,
            wxGetApp().plater()->get_camera(), hit, normal, nullptr
        )) {
...
void GLGizmoSlaSupports::select_point(int i) {
    ...
    if (i == AllPoints || i == NoPoints) {
        for (const auto &id : mo_idxs) {
            for (auto &point_and_selection : m_editing_cache[id])
                point_and_selection.selected = (i == AllPoints);
            m_selection_empty = (i == NoPoints);

            if (i == AllPoints && !m_editing_cache.empty())
                m_new_point_head_diameter = m_editing_cache[id][0].support_point.head_front_radius *
                    2.f;
        }
        ...
        // size_t temp_cache_id = 0;
        // while (i > m_editing_cache[m_hover_mo_id].size()) {
        //     i -= m_editing_cache[temp_cache_id++].size();
        // }
        ...
        m_editing_cache[m_hover_mo_id][i].selected = true;
        m_selection_empty = false;
        m_new_point_head_diameter =
            m_editing_cache[m_hover_mo_id][i].support_point.head_front_radius * 2.f;
    }
}
...
void GLGizmoSlaSupports::unselect_point(int i) {
    ...
    m_editing_cache[m_hover_mo_id][i].selected = false;
    m_selection_empty = true;
    for (const CacheEntry &ce : m_editing_cache[m_hover_mo_id]) {
        if (ce.selected) {
            m_selection_empty = false;
            break;
        }
...
void GLGizmoSlaSupports::editing_mode_discard_changes() {
    if (!m_editing_mode) {
        std::cout << "DEBUGGING: editing_mode_discard_changes called when out of editing mode!"
                  << std::endl;
        std::abort();
    }
    select_point(NoPoints);
...
void GLGizmoSlaSupports::editing_mode_apply_changes() {
    ...
    disable_editing_mode(
    ); // this leaves the editing mode undo/redo stack and must be done before the snapshot is taken
    ...
    for (const auto id : mo_idxs) {
        ...
        if (unsaved_changes(id)) {
            ...
            n_normal_cache.clear();
            for (const CacheEntry &ce : n_editing_cache)
                n_normal_cache.push_back(ce.support_point);

            mo->sla_points_status = sla::PointsStatus::UserModified;
            mo->sla_support_points.clear();
            mo->sla_support_points = n_normal_cache;
        }
    }
...
void GLGizmoSlaSupports::reload_cache() {
    ...
    std::set<unsigned int> mo_idxs = selection.get_object_idxs();
    for (const auto &id : mo_idxs) {
        const ModelObject *mo = selection.get_model()->objects[id];

        if (mo->sla_points_status == sla::PointsStatus::AutoGenerated ||
            mo->sla_points_status == sla::PointsStatus::Generating)
            get_data_from_backend();
        else if (mo->sla_support_points.size() != 0) {
            std::vector<sla::SupportPoint> &n_normal_cache = m_normal_cache[id];
            n_normal_cache.clear();
            for (const sla::SupportPoint &point : mo->sla_support_points)
                n_normal_cache.emplace_back(point);
        }
    }
}
...
bool GLGizmoSlaSupports::has_backend_supports() const {
    ...
    for (const auto &id : mo_idxs) {
        ...
        if (po && !po->is_step_done(slaposSupportPoints)) {
            result = false;
        }
    }
...
bool GLGizmoSlaSupports::on_mouse(const wxMouseEvent &mouse_event) {
    ...
    int v_id = m_parent.get_first_hover_volume_idx();
    if (v_id != -1) {
        m_hover_volumn_id = v_id;
    }
    // else
    //     m_hover_volumn_id = -1;
    ...
    if (mouse_event.Moving()) {
        ...
        if (m_hover_volumn_id != -1) {
            const GLVolumePtrs &gl_volumes = m_parent.get_volumes().volumes;
            auto hovered_idx = static_cast<size_t>(m_hover_volumn_id);
            if (hovered_idx < gl_volumes.size()) {
                const GLVolume *gl_volume = gl_volumes[hovered_idx];
                if (gl_volume) {
                    m_hover_mo_id = gl_volume->object_idx();
                }
            }
        ...
        if (m_editing_mode && m_hover_id == -1) {
            std::pair<Vec3f, Vec3f> pos_and_normal;
            if (unproject_on_mesh(mouse_pos, pos_and_normal)) {
                m_hover_position = pos_and_normal.first;
                m_hover_normal = pos_and_normal.second;
                m_parent.request_extra_frame(); // 请求重绘
            } else {
                m_hover_position.reset();
                m_hover_normal.reset();
            }
    ...
    if (!mouse_event.ShiftDown() && !mouse_event.AltDown() && use_grabbers(mouse_event))
        return true;
    ...
    if (mouse_event.LeftDown()) {
        ...
        if ((!control_down || grabber_contains_mouse) &&
            gizmo_event(
                SLAGizmoEventType::LeftDown, mouse_pos, mouse_event.ShiftDown(),
        ...
        if (m_parent.get_move_volume_id() != -1) {
            ...
            return true;
                   ...
                   gizmo_event(
                       SLAGizmoEventType::Dragging, mouse_pos, mouse_event.ShiftDown(),
                       mouse_event.AltDown(), false
                   )) {
            ...
            if (mouse_event.LeftIsDown())
                gizmo_event(
                    SLAGizmoEventType::LeftUp, mouse_pos, mouse_event.ShiftDown(),
        ...
        gizmo_event(
            SLAGizmoEventType::LeftUp, mouse_pos, mouse_event.ShiftDown(), mouse_event.AltDown(),
            mouse_event.CmdDown()
        );
        return true;
    ...
    for (const auto &id : mo_idxs) {
        ...
        if (po) {
            ...
            for (const sla::SupportPoint &p : po->get_support_points())
                n_normal_cache.emplace_back(
                    sla::SupportPoint{mat * p.pos, p.head_front_radius, p.type}
                );
...
// TODO: 生成支撑和底版之后，鼠标如果还停留在左边小窗口会出错
...
void GLGizmoSlaSupports::switch_to_editing_mode() {
    ...
    for (const auto &id : mo_idxs) {
        ...
        std::vector<CacheEntry> &n_editing_cache = m_editing_cache[id];
        std::vector<sla::SupportPoint> &n_normal_cache = m_normal_cache[id];
        n_editing_cache.clear();
        for (const sla::SupportPoint &sp : n_normal_cache)
            n_editing_cache.emplace_back(sp);
    }
    select_point(NoPoints);
    register_point_raycasters_for_picking();
    m_parent.set_as_dirty();
...
void GLGizmoSlaSupports::disable_editing_mode() {
    ...
    wxGetApp().plater()->get_notification_manager()->close_notification_of_type(
        NotificationType::QuitSLAManualMode
    );
}

bool GLGizmoSlaSupports::unsaved_changes(int id) {
    std::vector<CacheEntry> &n_editing_cache = m_editing_cache[id];
    std::vector<sla::SupportPoint> &n_normal_cache = m_normal_cache[id];

    if (n_editing_cache.size() != n_normal_cache.size())
        return true;

    for (size_t i = 0; i < n_editing_cache.size(); ++i)
        if (n_editing_cache[i].support_point != n_normal_cache[i])
            return true;
...
void GLGizmoSlaSupports::register_point_raycasters_for_picking() {
    ...
    size_t all_length = 0;
    for (auto it = m_editing_cache.begin(); it != m_editing_cache.end(); ++it) {
        const auto &value = it->second;
        all_length += value.size();
    }

    if (m_editing_mode && !m_editing_cache.empty()) {
        for (size_t i = 0; i < all_length; ++i) {
            m_point_raycasters.emplace_back(
                m_parent.add_raycaster_for_picking(
                    SceneRaycaster::EType::Gizmo, i, *m_sphere.mesh_raycaster, Transform3d::Identity()
                ),
                m_parent.add_raycaster_for_picking(
                    SceneRaycaster::EType::Gizmo, i, *m_cone.mesh_raycaster, Transform3d::Identity()
                )
            );
        }
        update_point_raycasters_for_picking_transform();
...
void GLGizmoSlaSupports::update_point_raycasters_for_picking_transform() {
    ...
    Selection &selection = m_parent.get_selection();
    std::set<unsigned int> mo_idxs = selection.get_object_idxs();
    size_t now_raycasters_id = 0;

    const auto &selected_volumes = volumes();
    for (GLVolume *v : selected_volumes.volumes) {
        if (mo_idxs.find(v->composite_id.object_id) == mo_idxs.end())
            continue;
        if (v->is_sla_pad() || v->is_sla_support())
            continue;

        Geometry::Transformation transformation(v->world_matrix());
        // const GLVolume *vol = m_parent.get_selection().get_first_volume();
        // Geometry::Transformation transformation(vol->world_matrix());

        ModelObject *mo = selection.get_model()->objects[v->composite_id.object_id];
        auto *inst = mo->instances[0];
        const SLAPrintObject *po = m_parent.sla_print()->get_print_object_by_model_object_id(mo->id(
        ));
        ...
        std::vector<CacheEntry> &n_editing_cache = m_editing_cache[v->composite_id.object_id];
        std::vector<sla::SupportPoint> &n_normal_cache = m_normal_cache[v->composite_id.object_id];

        for (size_t i = 0; i < n_editing_cache.size(); ++i) {
            const Transform3d support_matrix =
                Geometry::translation_transform(n_editing_cache[i].support_point.pos.cast<double>()
                ) *
                instance_scaling_matrix_inverse;

            // TODO: get_closest_point需要调这个函数矫正方向
            // if (n_editing_cache[i].normal == Vec3f::Zero())
            //     m_c->raycaster()->raycaster()->get_closest_point(
            //         n_editing_cache[i].support_point.pos, &n_editing_cache[i].normal
            //     );
            if (n_editing_cache[i].normal == Vec3f::Zero())
                m_volume_raycasters[v->composite_id.object_id]->get_raycaster()->get_closest_point(
                    n_editing_cache[i].support_point.pos, &n_editing_cache[i].normal
                );
            ...
            const Eigen::AngleAxisd aa(q);
            const Transform3d cone_matrix =
                transformation.get_matrix() * support_matrix * Transform3d(aa.toRotationMatrix()) *
                Geometry::assemble_transform(
                    (CONE_HEIGHT +
                     n_editing_cache[i].support_point.head_front_radius * RenderPointScale
                    ) * Vec3d::UnitZ(),
                    Vec3d(PI, 0.0, 0.0), Vec3d(CONE_RADIUS, CONE_RADIUS, CONE_HEIGHT)
                );
            m_point_raycasters[now_raycasters_id].second->set_transform(cone_matrix);

            const double radius = (double) n_editing_cache[i].support_point.head_front_radius *
                RenderPointScale;
            const Transform3d sphere_matrix = transformation.get_matrix() * support_matrix *
                Geometry::scale_transform(radius);
            m_point_raycasters[now_raycasters_id++].first->set_transform(sphere_matrix);
        }
    }
    ...
    //        //     );
    //        if (n_editing_cache[i].normal == Vec3f::Zero())
    //            m_volume_raycasters[id]->get_raycaster()->get_closest_point(
    //                n_editing_cache[i].support_point.pos, &n_editing_cache[i].normal
    //            );
...
SlaGizmoHelpDialog::SlaGizmoHelpDialog()
    ...
    auto note_text =
        new wxStaticText(this, wxID_ANY, _L("Note: some shortcuts work in (non)editing mode only."));
    ...
    for (const auto &[icon_name, description] : point_types) {
        ...
        desc->SetFont(font);
        gridsizer->Add(
            new wxStaticBitmap(this, wxID_ANY, ScalableBitmap(this, icon_name).bmp()), -1,
